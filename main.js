...
async function startStopwatch(rowElement, isRecovery = false) {
    ...
    if (!isRecovery) {
        ...
        fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({
            action: "start", rowId: rowId, name: rowElement.querySelector('.person-name').value,
            tvNum: rowElement.querySelector('.tv-number').value, startTime: startTime,
...
async function stopStopwatch(rowElement) {
    const rowId = rowElement.dataset.rowId;
    ...
    fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({
        action: "end", rowId: rowId, endTime: endTime, price: rowElement.querySelector('.priceBox').value
    })});
    saveData();
}
